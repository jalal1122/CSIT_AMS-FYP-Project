...
export default function LiveSessionSecurityModal({ isOpen, onClose, sessionId, currentConfig }) {
  ...
  useEffect(() => {
    if (currentConfig) {
      setRadius(currentConfig.radius || 50);
      setGeoEnabled(currentConfig.radius > 0);
      setManualApproval(currentConfig.manualApproval || false);
      setIpMatchEnabled(currentConfig.ipMatchEnabled ?? true);
      setDeviceLockEnabled(currentConfig.deviceLockEnabled ?? true);
      setQrRefreshRate(currentConfig.qrRefreshRate || 15);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await dispatch(updateSecuritySettings({
        sessionId,
        settings: { radius: finalRadius, manualApproval, ipMatchEnabled, deviceLockEnabled, qrRefreshRate }
      })).unwrap();
      onClose();
    } catch (err) {
      alert(err);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md relative z-10 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-semibold text-slate-700">Additional Options</label>
              <div className="flex items-center gap-2">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex gap-3">
          ...
          <button 
            onClick={handleSave}
            disabled={isSaving}
            className="flex-1 px-4 py-2 bg-sky-500 text-white rounded-lg font-medium flex items-center justify-center gap-2 hover:bg-sky-600 transition-colors shadow-sm shadow-sky-200 disabled:opacity-50"
          >
