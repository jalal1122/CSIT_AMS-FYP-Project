...
export default function EditDisciplineModal({ isOpen, onClose, onSubmit, discipline, departments }) {
  ...
  useEffect(() => {
    if (discipline) {
      setFormData({ 
        ...
        totalSemesters: discipline.totalSemesters, 
        departmentId: discipline.departmentId?._id || discipline.departmentId 
      });
    }
  }, [discipline]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(discipline._id, formData);
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md relative z-10 overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-800 text-lg">Edit Discipline</h3>
          </div>
          <button 
            onClick={onClose}
