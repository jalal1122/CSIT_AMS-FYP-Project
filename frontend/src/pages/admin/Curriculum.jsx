  ...
  return (
    <div className="space-y-4 sm:space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 shrink-0">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Curriculum Builder</h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">Design syllabus structures using drag-and-drop</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-4 w-full sm:w-auto">
          ...
          <button onClick={handleSaveCurriculum} className="btn-primary flex items-center justify-center gap-2 whitespace-nowrap w-full sm:w-auto text-xs sm:text-sm" disabled={!selectedDiscipline}>
      ...
      {!selectedDiscipline ? (
        <div className="flex-1 card border-dashed border-2 border-slate-200 flex flex-col items-center justify-center p-6 sm:p-12 min-h-[300px] sm:min-h-[400px]">
          <EmptyState 
            icon={BookOpen} 
        ...
        <div className="flex-1 grid grid-cols-1 xl:grid-cols-4 gap-4 sm:gap-6 min-h-[500px]">
          {/* Subject Pool */}
          <div className="xl:col-span-1 card p-0 flex flex-col overflow-hidden max-h-[420px] xl:max-h-[800px]">
          ...
          {/* Curriculum Canvas */}
          <div className="xl:col-span-3 card p-3 sm:p-6 overflow-x-auto bg-slate-50/50">
            <div className="flex gap-4 sm:gap-6 min-w-max pb-4 h-full">
              {Object.keys(semesters).map(semNum => (
                <div key={semNum} className="w-64 sm:w-72 flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden h-full shadow-sm">
                  <div className="p-3 sm:p-4 bg-slate-50/80 border-b border-slate-100 flex justify-between items-center">
                    <h4 className="font-bold text-slate-700 text-xs sm:text-sm">Semester {semNum}</h4>
                    <span className="bg-white border border-slate-200 text-slate-500 font-semibold text-[11px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full shadow-sm">
                      {semesters[semNum].reduce((acc, curr) => acc + curr.creditHours, 0)} Cr
                    </span>
                  </div>
                  <div className="p-2.5 sm:p-3 flex-1 overflow-y-auto space-y-2.5 sm:space-y-3 bg-slate-50/30 min-h-[200px]">
                    {semesters[semNum].length === 0 ? (
                      <div className="h-full min-h-[150px] flex items-center justify-center border-2 border-dashed border-slate-200 rounded-xl text-slate-400 text-xs sm:text-sm font-medium bg-white/50">
                        Drop subjects here
                      </div>
                    ) : (
                      semesters[semNum].map(sub => (
                         <div key={sub._id} className="bg-sky-50 p-2.5 sm:p-3 rounded-xl border border-sky-200 shadow-sm relative group">
                           <button onClick={() => handleRemoveSubjectFromSemester(sub, semNum)} className="absolute -top-2 -right-2 bg-white border border-slate-200 rounded-full p-1 text-slate-400 hover:text-rose-500 hover:border-rose-200 shadow-sm opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                             <X className="w-3 h-3" />
                           </button>
                           <div className="flex justify-between items-start mb-1.5">
